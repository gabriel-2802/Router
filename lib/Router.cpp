...
void Router::build_routes(char *file_name) {
    routing_table = new BinaryTrie();
...
void Router::build_static_arp_table(char *file_name, size_t len) {
...
optional<route_table_entry> Router::next_hop(ip_addr_t ip) {
    return routing_table->longest_prefix_match(ip);
}

bool Router::crc(iphdr *ip_hdr) {
...
bool Router::crc(icmphdr *icmp_hdr) {
...
void Router::handle_packet(Packet packet) {
...
void Router::handle_arp_packet(Packet packet) {
...
void Router::handle_ip_packet(Packet packet) {
...
void Router::forward_ip_packet(Packet packet) {
    iphdr *ip_hdr = (iphdr *)(packet.buff + sizeof(ether_header));
...
void Router::prepare_icmp_header(Packet& packet, uint8_t type) {
...
void Router::send_queued_packets(ip_addr_t ip, mac_addr_t mac) {
...
void Router::send_arp_reply(Packet packet) {
    arp_header *arp_hdr = (arp_header *)(packet.buff + sizeof(ether_header));
...
void Router::send_arp_request(ip_addr_t ip, int interface) {
    Packet packet;
...
void Router::send_packet(Packet packet) {
    size_t sent = 0;
...
void Router::send_icmp_echo_reply(Packet packet) {
